  ...
  var CSheetCell = function(opt) {
    ...
    * */
    this.set = function(state) {
      // x cellPrivate.state = state == 0 ? 1 : 0;
      cellPrivate.state = startCellType ? 0 : 1
  ...
  $.fn.TimeSheet = function(opt) {
    ...
    * */
    var initSheet = function() {
      thisSheet.html(`<thead class="calendar-head"><tr><th rowspan="8" class="week-td">${opt.data.sheetHead.name}</th> <th colspan="${opt.data.thColspan}">00:00 - 12:00</th> <th style='border-left: #e3e3e3 1px solid' colspan="${opt.data.thColspan}">12:00 - 24:00</th></tr> <tr><td colspan="${opt.data.tdColspan}">0</td><td colspan="${opt.data.tdColspan}">1</td><td colspan="${opt.data.tdColspan}">2</td><td colspan="${opt.data.tdColspan}">3</td><td colspan="${opt.data.tdColspan}">4</td><td colspan="${opt.data.tdColspan}">5</td><td colspan="${opt.data.tdColspan}">6</td><td colspan="${opt.data.tdColspan}">7</td><td colspan="${opt.data.tdColspan}">8</td><td colspan="${opt.data.tdColspan}">9</td><td colspan="${opt.data.tdColspan}">10</td><td colspan="${opt.data.tdColspan}">11</td><td colspan="${opt.data.tdColspan}">12</td><td colspan="${opt.data.tdColspan}">13</td><td colspan="${opt.data.tdColspan}">14</td><td colspan="${opt.data.tdColspan}">15</td><td colspan="${opt.data.tdColspan}">16</td><td colspan="${opt.data.tdColspan}">17</td><td colspan="${opt.data.tdColspan}">18</td><td colspan="${opt.data.tdColspan}">19</td><td colspan="${opt.data.tdColspan}">20</td><td colspan="${opt.data.tdColspan}">21</td><td colspan="${opt.data.tdColspan}">22</td><td style='border-right: 1px solid #ccc' colspan="${opt.data.tdColspan}">23</td></tr></thead>`)
    ...
    * */
    var initColHeads = function() {
      var colHeadHtml = '<tr>'
      // for (var i = 0, curColHead = ''; i <= sheetOption.data.dimensions[1]; i+=2) {
      // if (i === 0) {
      //     // curColHead = '<td></td>'
    ...
    * */
    ...
    var afterSelecting = function(ev, targetArea) {
      var curDom = $(ev.currentTarget)
      var key = $(ev.which)
      var targetState = Number()

      // 鼠标左键,将选定区域置1
      if (key[0] === 1) {
        targetState = 1
      } else if (key[0] === 3) { // 鼠标右键,将选定区域置0
        targetState = 0
      }
      if ((isSelecting && curDom.hasClass('TimeSheet-cell')) || (isColSelecting && curDom.hasClass('TimeSheet-colHead'))) {
    ...
    var eventBinding = function() {
      ...
      thisSheet.delegate('.TimeSheet-cell', 'mousedown.umsSheetEvent', function(ev) {
        startCellType = $(this).hasClass('TimeSheet-cell-selected')
